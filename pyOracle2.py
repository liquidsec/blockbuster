...
class Job:
    ...
    def __init__(self,blocksize,mode,debug,sourceString,name,ivMode,URL,httpMethod,additionalParameters,httpProxyOn,httpProxyIp,httpProxyPort,headers,iv,oracleMode,oracleText,vulnerableParameter,inputMode,cookies,encodingMode,postFormat,followRedirects,concurrency=5):
    ...
    def initialize_client(self):
        ...
        self.async_client = httpx.AsyncClient(proxy=proxy, verify=False, follow_redirects=self.followRedirects,
    ...
    async def makeRequestAsync(self, encryptedstring, progress=None):
        ...
        for attempt in range(max_retries):
            try:
                if method == "GET":
                    return await self.async_client.get(url, **kwargs)
                else:
                    return await self.async_client.post(url, **kwargs)
            except (httpx.ConnectError, httpx.TimeoutException, httpx.RemoteProtocolError) as e:
                if attempt < max_retries - 1:
                    wait = 2 ** attempt
...
async def async_main():
    ...
    # Proceed with a new job
        ...
        job = Job(blocksize,args.mode,args.debug,args.input,name,ivMode,URL,httpMethod,additionalParameters,httpProxyOn,httpProxyIp,httpProxyPort,headers,iv,oracleMode,oracleText,vulnerableParameter,inputMode,cookies,encodingMode,postFormat,followRedirects,concurrency)
        job.initialize()
