...
def make_mock_oracle(oracle: PaddingOracle, encoding_mode="base64", input_mode="parameter"):
...
class TestFullDecrypt:
    ...
    async def test_decrypt_short_plaintext_base64_firstblock(self):
        ...
        j = make_job(
            sourceString=source, mode="decrypt", ivMode="firstblock",
    ...
    async def test_decrypt_exact_block_boundary(self):
        ...
        combined = b''.join(j.solvedBlocks[i] for i in sorted(j.solvedBlocks.keys()))
        pad_len = combined[-1]
        stripped = combined[:-pad_len] if (1 <= pad_len <= 16 and combined[-pad_len:] == bytes([pad_len]) * pad_len) else combined
        assert stripped == plaintext
    ...
    async def test_decrypt_hex_encoding(self):
        ...
        j = make_job(
            sourceString=source, mode="decrypt", ivMode="firstblock",
            oracleMode="negative", oracleText="Invalid padding",
            concurrency=1, blocksize=16, encodingMode="hex",
        )
    ...
    async def test_decrypt_knownIV_mode(self):
        ...
        j = make_job(
            sourceString=source, mode="decrypt", ivMode="knownIV",
    ...
    async def test_decrypt_search_oracle_mode(self):
        ...
        j = make_job(
            sourceString=source, mode="decrypt", ivMode="firstblock",
            oracleMode="search", oracleText="Decryption successful",
            concurrency=1, blocksize=16, encodingMode="base64",
        )
    ...
    async def test_decrypt_with_async_concurrency(self):
        ...
        j = make_job(
            sourceString=source, mode="decrypt", ivMode="firstblock",
            oracleMode="negative", oracleText="Invalid padding",
            concurrency=25, blocksize=16, encodingMode="base64",
        )
...
class TestFullEncrypt:
    ...
    async def test_encrypt_short_plaintext(self):
        ...
        j = make_job(
            sourceString="Test", mode="encrypt", ivMode="firstblock",
            oracleMode="negative", oracleText="Invalid padding",
            concurrency=1, blocksize=16, encodingMode="base64",
        )
        ...
        joined = b''.join([joined, bytes([0] * 16)])  # append zero IV for firstblock mode
